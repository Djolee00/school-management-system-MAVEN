...
public class AdminCoursesController {
	...
	private void addEmptyCourse() {
		...
		resetCourses();
		JOptionPane.showMessageDialog(coursesView, "Please fill in information and click update button", "Info",
				JOptionPane.INFORMATION_MESSAGE);
	}

	private void deleteSelectedCourse() {
		if (coursesView.getTblCourses().getSelectedRow() == -1) {
			JOptionPane.showMessageDialog(coursesView, "Please select course you want to delete", "Message",
					JOptionPane.WARNING_MESSAGE);
			return;
		}

		if (JOptionPane.showConfirmDialog(coursesView, "Are you sure you want to delete this course?", "Confirmation",
			...
			if (temp.getId() != null) {
				...
				if (status == false) {
					JOptionPane.showMessageDialog(coursesView, "There are active groups, this course can't be deleted",
			...
			JOptionPane.showMessageDialog(coursesView, "Course's data successfully deleted", "Success",
					JOptionPane.INFORMATION_MESSAGE);
		}
	...
	private void updateSelectedCourse() {
		if (coursesView.getTblCourses().getSelectedRow() == -1) {
			JOptionPane.showMessageDialog(coursesView, "Please select course you want to update", "Message",
					JOptionPane.WARNING_MESSAGE);
			return;
		}

		if (JOptionPane.showConfirmDialog(coursesView, "Are you sure you want to update this course?", "Confirmation",
				JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
			Course temp = (Course) tableModel.getCourse(coursesView.getTblCourses().getSelectedRow());
			if (temp.getId() != null) {
				sendUpdateRequest(temp);
				JOptionPane.showMessageDialog(coursesView, "Course's data successfully updated", "Success",
						JOptionPane.INFORMATION_MESSAGE);
			} else {
				Long generatedId = sendSaveRequest(temp);
				temp.setId(generatedId);
				JOptionPane.showMessageDialog(coursesView, "Course's data successfully saved", "Success",
						JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	...
	private void sortCourses() {
		if (coursesView.getJrbLevel().isSelected()) {
			courses = courses.stream().sorted((c1, c2) -> c1.getLanguage().getLevel().toString()
					.compareTo(c2.getLanguage().getLevel().toString())).collect(Collectors.toList());
		}
		if (coursesView.getJrbLanguage().isSelected()) {
			courses = courses.stream()
					.sorted((c1, c2) -> c1.getLanguage().getName().compareTo(c2.getLanguage().getName()))
	...
	private void filterCourses() {
		...
		if (coursesView.getDateBegin().getDate() != null) {
			LocalDate startDate = coursesView.getDateBegin().getDate().toInstant().atZone(ZoneId.systemDefault())
					.toLocalDate();
			temp = temp.stream().filter(c -> c.getStartDate().isAfter(startDate) || c.getStartDate().isEqual(startDate))
					.collect(Collectors.toList());
		}

		if (coursesView.getDateEnd().getDate() != null) {
			LocalDate endDate = coursesView.getDateEnd().getDate().toInstant().atZone(ZoneId.systemDefault())
					.toLocalDate();
			temp = temp.stream().filter(c -> c.getEndDate().isBefore(endDate) || c.getEndDate().isEqual(endDate))
					.collect(Collectors.toList());
		}
		...
		if (courses.isEmpty()) {
			JOptionPane.showMessageDialog(coursesView, "No courses match criteria values", "Info",
					JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	private void manageGroupsForSelectedCourse() {
		if (coursesView.getTblCourses().getSelectedRow() == -1) {
			JOptionPane.showMessageDialog(coursesView, "Please select course, whose groups you want to manage",
					"Message", JOptionPane.WARNING_MESSAGE);
			return;
		}
		Course temp = (Course) tableModel.getCourse(coursesView.getTblCourses().getSelectedRow());
		if (temp.getId() == null) {
			JOptionPane.showMessageDialog(coursesView, "Please select course, click Update course firt", "Message",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
	...
	private void getAllLanguages() {
		...
		try {
			...
			JOptionPane.showMessageDialog(coursesView, "Error getting languages' data. Try again later!", "Error",
	...
	private List<Course> getAllCourses() {
		...
		try {
			...
			JOptionPane.showMessageDialog(coursesView, "Error getting courses' data. Try again later!", "Error",
					JOptionPane.ERROR_MESSAGE);
			coursesView.dispose();
			System.exit(0);
		}
	...
	private void prepareDateColumns() {
		coursesView.getTblCourses().getColumnModel().getColumn(1)
				.setCellEditor(new JDateChooserCellEditor(new JCheckBox()));
		coursesView.getTblCourses().getColumnModel().getColumn(2)
				.setCellEditor(new JDateChooserCellEditor(new JCheckBox()));
	}

	private void prepareCapacityColumn() {
		coursesView.getTblCourses().getColumnModel().getColumn(3)
				.setCellEditor(new DefaultCellEditor(createTextFieldForCapacityCell()));
	}
	...
	private void sendUpdateRequest(Course temp) {
		try {
			...
			JOptionPane.showMessageDialog(coursesView, "Error updating course's data. Try again later!", "Error",
					JOptionPane.ERROR_MESSAGE);
			coursesView.dispose();
			System.exit(0);
		}
	...
	private boolean sendDeleteRequest(Course temp) {
		try {
			...
			JOptionPane.showMessageDialog(coursesView, "Error deleting course's data. Try again later!", "Error",
					JOptionPane.ERROR_MESSAGE);
			coursesView.dispose();
			System.exit(0);
			return false;
