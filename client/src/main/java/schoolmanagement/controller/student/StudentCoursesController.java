...
public class StudentCoursesController {
	...
	private void filterCourses() {
		...
		if (courses.isEmpty()) {
			JOptionPane.showMessageDialog(coursesView, "There are no courses with given input values", "Info",
	...
	private void chooseCourses() {
		...
		if (coursesView.getTblCourses().getSelectedRows().length == 0) {
			JOptionPane.showMessageDialog(coursesView, "Please select one or more courses.", "Warning",
					JOptionPane.WARNING_MESSAGE);
			return;
		}

		if (JOptionPane.showConfirmDialog(coursesView, "Are you sure you want to enroll in these courses?",
			...
			if (status == true) {
				JOptionPane.showMessageDialog(coursesView, "You have been successfully added to selected courses!",
						"Success", JOptionPane.INFORMATION_MESSAGE);
				new StudentHomeController();
			} else {
				JOptionPane.showMessageDialog(coursesView,
						"Error while enrolling student in selected courses. Please try again!", "Error",
	...
	private List<Course> getStudentsUnselectedCourses() {
		...
		try {
			...
			JOptionPane.showMessageDialog(coursesView, "Error getting student's courses. Try again later!", "Error",
					JOptionPane.ERROR_MESSAGE);
			coursesView.dispose();
			System.exit(0);
		}
