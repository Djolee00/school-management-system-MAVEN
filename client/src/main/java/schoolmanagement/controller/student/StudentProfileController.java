...
public class StudentProfileController {
	...
	private void populateFields() {
		...
		profileView.getTxtBirthDate()
				.setValue(Date.from(student.getBirthdate().atStartOfDay(ZoneId.systemDefault()).toInstant()));
		profileView.getTxtCreationDate()
				.setValue(Date.from(student.getCreationDate().atStartOfDay(ZoneId.systemDefault()).toInstant()));
	...
	private List<CourseEnrollment> getStudentCourses() {
		...
		try {
			...
			JOptionPane.showMessageDialog(profileView, "Error getting student's courses. Try again later!", "Error",
	...
	private void courseSelection() {
		...
		CourseGroup tempGroup = tempCourseEnrollment.getCourse().getCourseGroups().get(0); // we only have group in
																							// which student exist (if
																							// he is added in any group)
		...
		if (tempGroup != null) {
			...
			for (Tutor temp : tempGroup.getTutors()) {
				profileView.getTxtTutors().setText(
						profileView.getTxtTutors().getText() + temp.getFirstName() + " " + temp.getLastName() + "\n");
			}
