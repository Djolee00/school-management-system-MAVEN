		...
		try {
			...
			case 0 -> {
				if (((String) aValue).equals("")) {
					JOptionPane.showMessageDialog(null, "Please insert name of course", "Error",
			...
			case 1 -> {
				...
				if (startDate.isAfter(temp.getEndDate()) || startDate.isEqual(temp.getEndDate())) {
					JOptionPane.showMessageDialog(null, "Start date can't be after end date", "Error",
							JOptionPane.WARNING_MESSAGE);
				} else {
			...
			case 2 -> {
				...
				if (endDate.isBefore(temp.getStartDate()) || endDate.isEqual(temp.getStartDate())) {
					JOptionPane.showMessageDialog(null, "End date can't be before start date", "Error",
							JOptionPane.WARNING_MESSAGE);
				} else {
